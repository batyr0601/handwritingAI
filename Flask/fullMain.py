...
modelDigits = tf.keras.models.load_model('model.emnistDigits')
modelLetters = tf.keras.models.load_model('model.emnistLetters')

def processDataUrl(file):
...
def digits():
    if request.method == 'POST':
        ...
        finalImg = processDataUrl(file)
        prediction = modelDigits.predict(finalImg)
        predictionDict = {}
        ...
        return render_template('index.html',
                                ...
                                ans3=f"{predictionDict['ans2'][0]} {predictionDict['ans2'][1]}", content = 'Digit detector')
    else:
        return render_template('index.html', content = 'Digit detector')
...
def letters():
    if request.method == 'POST':
        ...
        finalImg = processDataUrl(file)
        prediction = modelLetters.predict(finalImg)
        predictionDict = {}
        ...
        return render_template('index.html',
                            ans1=f"{predictionDict['ans0'][0]} {predictionDict['ans0'][1]}",
                            ans2=f"{predictionDict['ans1'][0]} {predictionDict['ans1'][1]}",
                            ans3=f"{predictionDict['ans2'][0]} {predictionDict['ans2'][1]}", content = 'Letter detector')

    else:
        return render_template('index.html', content = 'Letter detector')
